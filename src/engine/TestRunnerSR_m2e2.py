...
class JointRunnerTest(object):
    def __init__(self):
        ...
        parser.add_argument('--keep_events', help="keep the sentence having events larger than int(keep_events)",
                            default=1, type=int)
        parser.add_argument('--keep_events_sr', help="image side keep the sentence having events larger than int(keep_events)",
                            default=1, type=int)
...
def joint_test_batch(model_g, batch_g, device, transform, img_dir,
                    sr_noun_i2s, sr_verb_i2s, sr_role_i2s,
                    role_masks, image_gt, verb2type, role2role, vision_result,
    ...
    ######################### Joint ######################
    batch_unpacked = unpack_m2e2(batch_g, device, transform, img_dir, None,
    ...
    sr_verb_, sr_role_, heatmap = \
        model_g.predict_image_only(image_id, image, role_masks,
                                   add_object=load_object,
                                   bbox_entities_id=bbox_entities_id_all,
                                   bbox_entities_region=bbox_entities_region_all,
                                   bbox_entities_label=bbox_entities_label_all,
                                   object_num_batch=object_num_all
                                   )
    ...
    #         verb_name = sr_verb_i2s[sr_verb_[batch_id][img_idx]]
    #         if verb_name not in verb2type:
    #             visual_writer.write(f'No ACE Event, {verb_name} <br>')
    #             continue
    #         ace_type_name = verb2type[verb_name]
    #         visual_writer.write(f'sr_verb_, {verb_name}, {ace_type_name} <br>')
    #         if img_id.replace('.jpg', '') in image_gt:
    ...
    #                 sr_role_name = sr_role_i2s[role_idx]
    #                 ace_role_name = role2role[verb_name][sr_role_name]
    #                 visual_writer.write(f'{ace_role_name} {sr_role_name} = {sr_noun_i2s[sr_role_[batch_id][img_idx][role_idx]]} <br>')
    #         image_done.add(img_id)
    #
    #     visual_writer.flush()
    #     visual_writer.close()
